...
import {
	MemberCasual,
	RideableType,
	type ServerResponseAll as ServerResponseAllPbEs,
	ServerResponseAllSchema,
} from "./protobuf_es_trip.js";
...
export const protobufEs: Deserializer<ServerResponseAllPbEs> = {
	...
	materializeUnverifiedAsPojo: (deserialized: ServerResponseAllPbEs) => {
	...
	scanForIdProperty: (deserialized: ServerResponseAllPbEs, targetId: string) => {
		const trip = deserialized.trips.find((trip) => trip.rideId === targetId);
		return trip !== undefined;
	},
	materializeVerifedAsPojo: (deserialized: ServerResponseAllPbEs): ServerResponseAll => {
