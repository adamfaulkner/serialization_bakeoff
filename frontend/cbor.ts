...
export const cbor: Deserializer<any> = {
	...
	materializeUnverifiedAsPojo: (deserialized: any) => {
		return {
			trips: deserialized.trips.map((trip: any) => ({
				...
				rideableType: trip.rideableType === "classic_bike" ? RideableType.classic : RideableType.electric,
				memberCasual: trip.memberCasual === "member" ? MemberCasual.member : MemberCasual.casual,
			})),
		};
	},
	...
	materializeVerifedAsPojo: function (deserialized: any): ServerResponseAll {
		...
		// json deserializer.
		const ajv = new Ajv();
		const result = ajv.validate(serverResponseAllReceivedJsonAjvSchema, deserialized);
		if (!result) {
			debugger;
