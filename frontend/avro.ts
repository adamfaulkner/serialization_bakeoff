...
import {  RideableType, ServerResponseAll, Trip } from "./trip.js";
...
const currentAvroType = productionAvsc.Type.forSchema(avroSchema as any, { logicalTypes: { 'timestamp-millis': CurrentDateType, 'missing': CurrentMissingType}, typeHook: currentTypeHook, });

const newAvroType = newAvsc.Type.forSchema(avroSchema as any, { logicalTypes: { 'timestamp-millis': NewDateType, 'missing': NewMissingType}, typeHook: newTypeHook, });
...
export const currentAvro: Deserializer<ServerResponseAll> = {
  ...
  deserializeAll: function (data: Uint8Array): ServerResponseAll{
    ...
    const result = currentAvroType.decode(Buffer.from(data.buffer as unknown as ArrayBuffer, data.byteOffset, data.byteLength)).value;
